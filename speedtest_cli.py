...
def speedtest():
    ...
    parser.add_argument('--file', help='Writes single-line output to a '
                            ' specified log file; no output to console.')
    ...
    # Start building the output file string with the DATE, TIME, and 
    # local HOST name of the test computer
    filestring = '%s|%s|%s|' % (time.strftime("%d-%b-%Y"), time.strftime("%H:%M.%S"), socket.gethostname())

    if args.file:   ## Using file argument overrides simple argument
        args.simple = False
        filenameexists = True
    else:
    ...
    if not (args.simple or args.file):
    ...
    if args.list or args.server:
        ...
        if args.list:
            ...
            for server in servers:
                ...
                # Some servers do not have a host, which results in a KeyError
                # this solution appends the host to the line without the use of
                # additional modules
                line +=  server.get('host', "No host")
    ...
    ## SOURCE
    filestring += '%(isp)s (%(ip)s)|' % config['client'] 
    ...
    if not (args.simple or args.file):
     ...
     # TARGET, DISTANCE, and PING
    filestring += ('%s' % best.get('host', "No host"))
    filestring += (' (%(sponsor)s: %(name)s)|%(d)0.2f|%(latency)s|' % best)
    ...
    dlspeed = downloadSpeed(urls, (args.simple or filenameexists))
    ...
    # DOWNLOAD
    filestring += ('%0.2f|' % ((dlspeed / 1000 / 1000) * args.units[1]))
    ...
    ulspeed = uploadSpeed(best['url'], sizes, (args.simple or filenameexists))
    ...
    if args.file:
        header = 'DATE|TIME|HOST|SOURCE|TARGET|DISTANCE(km)|PING(ms)|DOWNLOAD(M%s/s)|UPLOAD(M%s/s)\n' % (args.units[0], args.units[0])

        ## For Debugging
        # print_('Writing the output to file (%s).....' % args.file) 
        # print_(filestring)
